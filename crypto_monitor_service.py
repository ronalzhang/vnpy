...
logger.add(
    os.path.join(LOG_DIR, "crypto_service_{time}.log"), 
    rotation="100 MB", 
    level="INFO",
...
# 价格日志文件
PRICE_LOG_FILE = os.path.join(LOG_DIR, f"price_diff_log_{datetime.now().strftime('%Y%m%d')}.csv")
...
def calculate_price_differences(prices):
    ...
    for symbol, data in prices.items():
        row = {"symbol": symbol, "timestamp": datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
        max_price = 0
        min_price = float('inf')
        max_exchange = ""
        min_exchange = ""
...
def log_price_differences(diff_data):
    """记录价格差异到CSV文件"""
    try:
        if not diff_data:
            return
        ...
        # 创建或追加到CSV文件
        file_exists = os.path.isfile(PRICE_LOG_FILE)
        ...
        columns.extend(["min_exchange", "min_price", "max_exchange", "max_price", "price_diff", "price_diff_pct"])
        ...
        # 写入CSV
        df.to_csv(PRICE_LOG_FILE, mode='a', header=not file_exists, index=False)
        
    except Exception as e:
        logger.error(f"记录价格差异失败: {e}")
...
def check_balances():
    ...
    for name, exchange in exchanges.items():
        try:
            ...
            # 记录余额到日志
            balance_str = ", ".join([f"{currency}: {amount}" for currency, amount in non_zero.items()])
            logger.info(f"{name.upper()} 账户余额: {balance_str}")
...
def has_sufficient_balance(symbol, exchange_name, required_amount):
    ...
    if exchange_name not in exchange_balances:
        logger.error(f"无法获取 {exchange_name} 的余额信息")
        return False
    ...
    if not has_enough:
        logger.warning(
            f"{exchange_name} 余额不足 - 需要: {base_currency}={required_amount}, "
            ...
            if price_diff_pct <= CLOSE_THRESHOLD:
                try:
                    ...
                    if not has_sufficient_balance(symbol, arb_info["buy_exchange"], amount):
                        logger.warning(f"平仓套利失败: {symbol} - {arb_info['buy_exchange']} 余额不足")
                        continue
                        
                    if not has_sufficient_balance(symbol, arb_info["sell_exchange"], 0):  # 卖出时只需检查USDT余额是否充足
                        logger.warning(f"平仓套利失败: {symbol} - {arb_info['sell_exchange']} 余额不足")
                        continue
                    ...
                    # 记录交易详情
                    logger.info(f"平仓交易详情 - 买入: {buy_result}, 卖出: {sell_result}")
        ...
        # 开仓逻辑
            ...
            try:
                ...
                if not has_sufficient_balance(symbol, buy_exchange_name, 0):  # 买入时只需检查USDT余额是否充足
                    logger.warning(f"开仓套利失败: {symbol} - {buy_exchange_name} USDT余额不足")
                    continue
                    
                if not has_sufficient_balance(symbol, sell_exchange_name, amount):
                    logger.warning(f"开仓套利失败: {symbol} - {sell_exchange_name} {symbol.split('/')[0]}余额不足")
                    continue
                ...
                # 记录交易详情
                logger.info(f"开仓交易详情 - 买入: {buy_result}, 卖出: {sell_result}")
...
def reconnect_exchanges():
    ...
    if success:
        logger.info(f"成功重新连接 {len(exchanges)} 个交易所")
        return True
    else:
        logger.error("重新连接交易所失败")
        return False


def monitor_loop(enable_trading=False):
    """价格监控循环"""
    reconnect_interval = 3600  # 每小时重新连接一次交易所
    ...
    while True:
        try:
            ...
            # 记录价格差异到CSV
            log_price_differences(diff_data)
            ...
            try:
                ...
                logger.error("重新连接交易所失败，将在下一轮继续尝试")
...
def main():
    """主函数"""
    logger.info("="*50)
    logger.info("加密货币跨交易所差价监控和自动套利服务启动")
    logger.info("="*50)
    ...
    if not init_exchanges():
        logger.error("初始化交易所失败，请检查配置文件")
        return
    
    logger.info(f"成功连接 {len(exchanges)} 个交易所")
    logger.info(f"监控 {len(SYMBOLS)} 个交易对: {', '.join(SYMBOLS)}")
    
    # 记录套利参数
    if enable_trading:
        logger.info("已启用自动套利交易")
        logger.info(f"套利开仓阈值: {ARBITRAGE_THRESHOLD*100:.1f}%")
        logger.info(f"套利平仓阈值: {CLOSE_THRESHOLD*100:.1f}%")
    else:
        logger.info("仅监控模式，不会执行自动交易")
